...
import { getTodos, insertTodo, updateTodo } from '@/lib/supabase/queries'
...
export default function TodoPage() {
    ...
    const handleSave = async () => {
        ...
        setSaving(true)
        try {
            await insertTodo({ titolo: form.titolo, descrizione: form.descrizione || null, priorita: form.priorita, data_scadenza: form.data_scadenza || null })
            setDialogOpen(false); setForm({ titolo: '', descrizione: '', priorita: 'media', data_scadenza: '' })
            toast.success('Attività aggiunta'); await loadData()
                ...
                <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                    ...
                    <DialogContent className="max-w-md">
                        ...
                        <div className="space-y-4 pt-2">
                            ...
                            <Button onClick={handleSave} disabled={saving} className="w-full touch-target rounded-xl font-semibold">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Aggiungi'}</Button>
                        </div>
                    </DialogContent>
                </Dialog>
            </div>
                        ...
                        {done.map(t => (
                            <Card key={t.id} className="shadow-sm opacity-60">
                                <CardContent className="p-3 flex items-center gap-3">
                                    ...
                                    <p className="text-sm line-through">{t.titolo}</p>
                                </CardContent>
                            </Card>
                        ))}
                    </div>
